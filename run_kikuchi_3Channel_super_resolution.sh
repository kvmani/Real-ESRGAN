...
# Script: train_realsrgan.sh
#
# Description: This script is used to train Real-ESRGAN model with PyTorch.
#              It sets up the environment, activates the conda environment,
#              and starts the training process.
#
# Usage: train_realsrgan.sh [--cuda_id <CUDA_VISIBLE_DEVICES>]
#
# Options:
...
cd ~/ml_works/Real-ESRGAN-master/Real-ESRGAN-master/scripts
pwd

# python ../test_gpu.py
echo Running the final line
#torchrun --nproc_per_node=$n_GPUs --master_port=$MASTER_PORT ../realesrgan/train.py -opt ../options/train_realesrgan_x4plus.yml --debug
#CUDA_VISIBLE_DEVICES=0,1,2,3 \
python -m torch.distributed.launch --nproc_per_node=4 --master_port=$MASTER_PORT ../realesrgan/train.py -opt ../options/train_kikuchi_3Channel_realesrnet_x4plus.yml --launcher pytorch
